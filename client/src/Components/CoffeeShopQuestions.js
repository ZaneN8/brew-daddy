...
const CoffeeShopQuestions = ({questionsShopId}) => {
  ...
  const getQuestions = async () => {
    ...
    try {
      let res = await axios.get(`/api/coffee_shops/${questionsShopId}/questions`);
  ...
  const deleteQuestion = async(id) =>{
    try{
      let res = await axios.delete(`/api/coffee_shops/${questionsShopId}/questions/${id}`, setQuestions) 
  ...
  return (
    <div>
      <br />
      {showCQuestions && <QuestionForm questionProp={questions.id} questionsShopId={questionsShopId}/>}
